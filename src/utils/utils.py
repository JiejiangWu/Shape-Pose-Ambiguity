...
def save_named_checkpoint(name,path, iteration, model_G, model_D,optimizer_G,optimizer_D,Losses_during_iteration):
    state = {'iteration': iteration,
             'model_G': model_G,
    ...
    tempfilename = path + '/Temp.pth.tar'
    torch.save(state, filename)
    torch.save(state, tempfilename)
